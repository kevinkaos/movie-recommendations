...
export default {
    configuration: {
        ...
        getConfigs: (type = 'countries') => api({
            method: 'GET',
            url: `${baseUrl}/configuration/${type}?api_key=${apiKey}`
        })
    },
    movie: {
        getMovies: (selection = 'popular', page = 1, language = 'en-US', region = 'US') => api({
            method: 'GET',
            url: `${baseUrl}/movie/${selection}?api_key=${apiKey}&language=${language}&region=${region}&page=${page}`
        }),
        getMovieDetails: (movieId = 0, language = 'en-US') => api({
            method: 'GET',
